        ...
        <div className="p-8 flex flex-col h-full space-y-8">
          ...
          <nav className="flex-grow space-y-6 custom-scrollbar overflow-y-auto pr-2">
            <div className="space-y-2">
               <button onClick={() => toggleCategory('DOCUMENTOS')} className="w-full flex items-center justify-between px-4 py-2 text-indigo-400 font-black text-[10px] uppercase tracking-[0.2em] hover:text-white transition-colors">
               ...
               {expandedCats.DOCUMENTOS && (
                 <div className="space-y-1.5 animate-in slide-in-from-top-2 duration-300">
                    <button onClick={() => handleNavigate('vehiculos')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeView === 'vehiculos' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                      <LayoutDashboard size={18}/> Vehículos
                    </button>
                    <button onClick={() => handleNavigate('conductores')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeView === 'conductores' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
            ...
            <div className="space-y-2">
               <button onClick={() => toggleCategory('NEUMATICOS')} className="w-full flex items-center justify-between px-4 py-2 text-indigo-400 font-black text-[10px] uppercase tracking-[0.2em] hover:text-white transition-colors">
               ...
               {expandedCats.NEUMATICOS && (
                 <div className="space-y-1.5 animate-in slide-in-from-top-2 duration-300">
                    <button onClick={() => handleNavigate('calibraciones')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeView === 'calibraciones' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
            ...
            <div className="space-y-2">
               <button onClick={() => toggleCategory('GESTION')} className="w-full flex items-center justify-between px-4 py-2 text-indigo-400 font-black text-[10px] uppercase tracking-[0.2em] hover:text-white transition-colors">
               ...
               {expandedCats.GESTION && (
                 <div className="space-y-1.5 animate-in slide-in-from-top-2 duration-300">
                    <button onClick={() => handleNavigate('kilometrajes')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeView === 'kilometrajes' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                      <Gauge size={18}/> Kilometrajes
                    </button>
                    <button onClick={() => handleNavigate('novedades')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeView === 'novedades' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                      <ClipboardList size={18}/> Novedades
                    </button>
                    <button onClick={() => handleNavigate('fives')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeView === 'fives' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                      <Shield size={18}/> 5S Auditoría
                    </button>
                    <button onClick={() => handleNavigate('inventario')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeView === 'inventario' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
        ...
        <header className="bg-white border-b p-6 flex justify-between items-center shadow-sm relative z-40">
           <div className="flex items-center gap-4">
             ...
             <div className="bg-slate-50 border rounded-xl px-4 py-3 flex items-center gap-3 w-64 md:w-96">
               ...
               <input type="text" placeholder={activeView === 'inventario' ? "BUSCAR ARTÍCULO..." : "BUSCAR PLACA..."} className="bg-transparent font-black uppercase text-xs outline-none flex-grow" value={searchTerm} onChange={e => setSearchTerm(e.target.value.toUpperCase())} />
             </div>
           </div>
           
           <div className="flex items-center gap-4">
              {/* Ocultar Semana en Novedades y Documentos */}
              {activeView !== 'novedades' && activeView !== 'vehiculos' && activeView !== 'conductores' && (
                <div className="hidden md:flex flex-col items-end animate-in fade-in duration-300">
          ...
          {activeView === 'vehiculos' && (
            <div className="space-y-8 max-w-7xl mx-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                 <button onClick={() => setStatsFilter('all')} className={`bg-white p-8 rounded-[2.5rem] border text-left transition-all hover:shadow-lg ${statsFilter === 'all' ? 'border-[#0f172a] border-4 ring-4 ring-[#0f172a]/10' : 'shadow-sm'}`}>
                 ...
                 <button onClick={() => setStatsFilter(statsFilter === 'soat_expired' ? 'all' : 'soat_expired')} className={`bg-white p-8 rounded-[2.5rem] border text-left transition-all hover:shadow-lg ${statsFilter === 'soat_expired' ? 'border-rose-500 border-4 ring-4 ring-rose-500/10' : 'shadow-sm border-rose-100'}`}>
                 ...
                 <button onClick={() => setStatsFilter(statsFilter === 'rtm_warning' ? 'all' : 'rtm_warning')} className={`bg-white p-8 rounded-[2.5rem] border text-left transition-all hover:shadow-lg ${statsFilter === 'rtm_warning' ? 'border-amber-500 border-4 ring-4 ring-amber-500/10' : 'shadow-sm border-amber-100'}`}>
                 ...
                 <button onClick={() => setStatsFilter(statsFilter === 'extinguisher_alert' ? 'all' : 'extinguisher_alert')} className={`p-8 rounded-[2.5rem] text-left transition-all hover:shadow-xl ${statsFilter === 'extinguisher_alert' ? 'bg-indigo-800 border-4 border-white shadow-indigo-500/50' : 'bg-indigo-600 shadow-xl'}`}>
                    <p className="text-[10px] font-black text-indigo-200 uppercase tracking-widest mb-2">Extintores Alerta</p>
                    <p className="text-4xl font-black text-white">{vehicles.filter(v => calculateStatus(v.extinguisher.expiryDate) !== 'active').length}</p>
                 </button>
              </div>
              ...
              {statsFilter !== 'all' && (
                <div className="flex items-center justify-between bg-slate-100 px-6 py-4 rounded-2xl mb-6 animate-in fade-in slide-in-from-top-4">
                  ...
                  <button onClick={() => setStatsFilter('all')} className="flex items-center gap-2 px-4 py-2 bg-white rounded-xl text-[10px] font-black text-rose-500 uppercase tracking-widest shadow-sm hover:bg-rose-500 hover:text-white transition-all">
          ...
          {activeView === 'novedades' && (
            <div className="space-y-8 max-w-7xl mx-auto">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <button onClick={() => setReportStatusFilter('TODOS')} className={`bg-white p-8 rounded-[2.5rem] border text-left transition-all hover:shadow-lg ${reportStatusFilter === 'TODOS' ? 'border-[#0f172a] border-4 ring-4 ring-[#0f172a]/10' : 'shadow-sm'}`}>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Total Casos</p>
                    <p className="text-4xl font-black text-[#0f172a]">{reports.filter(r => (filterCd === 'all' || r.cd === filterCd)).length}</p>
                  </button>
                  <button onClick={() => setReportStatusFilter('ABIERTO')} className={`bg-white p-8 rounded-[2.5rem] border text-left transition-all hover:shadow-lg ${reportStatusFilter === 'ABIERTO' ? 'border-rose-500 border-4 ring-4 ring-rose-500/10' : 'shadow-sm border-rose-100'}`}>
                    <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-2">Novedades Abiertas</p>
                    <p className="text-4xl font-black text-rose-600">{reports.filter(r => (filterCd === 'all' || r.cd === filterCd) && r.status === 'ABIERTO').length}</p>
                  </button>
                  <button onClick={() => setReportStatusFilter('CERRADO')} className={`bg-white p-8 rounded-[2.5rem] border text-left transition-all hover:shadow-lg ${reportStatusFilter === 'CERRADO' ? 'border-emerald-500 border-4 ring-4 ring-emerald-500/10' : 'shadow-sm border-emerald-100'}`}>
                    <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-2">Cierres Exitosos</p>
                    <p className="text-4xl font-black text-emerald-600">{reports.filter(r => (filterCd === 'all' || r.cd === filterCd) && r.status === 'CERRADO').length}</p>
                  </button>
               </div>

               <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-[#0f172a] p-10 rounded-[3rem] text-white gap-6">
                 <div>
                    <h2 className="text-3xl font-black uppercase tracking-tighter">Control de Taller</h2>
                    <p className="text-indigo-400 text-[10px] font-black uppercase tracking-widest mt-2">Filtro Actual: <span className="text-white">{reportStatusFilter}</span></p>
                 </div>
                 <div className="flex flex-wrap items-center gap-4">
                    <button onClick={() => setShowReportForm(true)} className="px-8 py-5 bg-indigo-600 rounded-2xl font-black text-xs tracking-widest flex items-center gap-3 hover:bg-indigo-700 transition-all shadow-xl">
                      <Plus /> ABRIR REPORTE
                    </button>
                 </div>
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 {reports
                   .filter(r => (filterCd === 'all' || r.cd === filterCd) && (r.plate.includes(searchTerm)) && (reportStatusFilter === 'TODOS' || r.status === reportStatusFilter))
                   .sort((a,b) => b.date.localeCompare(a.date))
                   .map(r => (
                     <ReportCard key={r.id} report={r} onViewDoc={(url, t) => setViewDoc({url, title: t})} onManageClosure={setClosingReport} />
                   ))}
                 {reports.filter(r => (filterCd === 'all' || r.cd === filterCd) && (r.plate.includes(searchTerm)) && (reportStatusFilter === 'TODOS' || r.status === reportStatusFilter)).length === 0 && (
                    <div className="col-span-full py-20 text-center flex flex-col items-center justify-center bg-white rounded-[4rem] border-2 border-dashed border-slate-200">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">No hay reportes con estado {reportStatusFilter}</p>
                    </div>
                 )}
               </div>
            </div>
            ...
            <div className="space-y-10 max-w-7xl mx-auto">
               <div className="flex justify-between items-center bg-indigo-900 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
                 ...
                 <div className="relative z-10">
                   ...
                   <p className="text-indigo-300 text-[10px] font-black uppercase tracking-widest mt-2 flex items-center gap-2"><Box size={14} /> Control de Suministros</p>
                 </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {[{ title: 'Aceite 15W40', stock: 12, unit: 'Galones', status: 'warning', icon: <Flame className="text-amber-500" /> }, { title: 'Llantas 295/80', stock: 4, unit: 'Unidades', status: 'critical', icon: <Disc className="text-rose-500" /> }, { title: 'Filtros Aire', stock: 45, unit: 'Unidades', status: 'ok', icon: <Settings2 className="text-emerald-500" /> }].map((item, idx) => (
