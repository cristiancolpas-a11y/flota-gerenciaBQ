                ...
                <div className={`transition-all duration-300 ease-in-out ${expandedSections[section.id] ? 'max-h-96 opacity-100 py-3' : 'max-h-0 opacity-0 overflow-hidden py-0'}`}>
                  <nav className="space-y-1.5 px-4">
                    {section.items.map((item) => (
                      ...
                      >
                        {item.icon}
                        <span className="uppercase tracking-widest label:none leading-none">{item.label}</span>
                      </button>
                    ))}
                  </nav>
                </div>
        ...
        <div className="flex-grow p-10 overflow-y-auto bg-[#f8fafc]">
          ...
          {activeView === 'vehiculos' && (
            <div className="space-y-12 animate-in fade-in duration-500 max-w-[1600px] mx-auto">
              <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
                 <button onClick={() => setVehicleStatusFilter('all')} className={`flex flex-col items-center justify-center gap-4 p-8 rounded-[3.5rem] border-2 transition-all duration-300 shadow-sm ${vehicleStatusFilter === 'all' ? 'bg-[#0f172a] text-white border-[#0f172a] scale-105 shadow-xl' : 'bg-white hover:border-indigo-200'}`}>
                    <div className={`w-16 h-16 rounded-3xl flex items-center justify-center transition-colors ${vehicleStatusFilter === 'all' ? 'bg-indigo-600' : 'bg-indigo-50 text-indigo-600'}`}><Truck size={32} /></div>
                    <div className="text-center"><h4 className="text-4xl font-black tracking-tighter leading-none">{vehicleStats.total}</h4><p className={`text-[9px] font-black uppercase tracking-[0.2em] mt-2 ${vehicleStatusFilter === 'all' ? 'text-indigo-400' : 'text-slate-400'}`}>TODOS</p></div>
                 </button>
                 <button onClick={() => setVehicleStatusFilter('soat')} className={`flex flex-col items-center justify-center gap-4 p-8 rounded-[3.5rem] border-2 transition-all duration-300 shadow-sm ${vehicleStatusFilter === 'soat' ? 'bg-[#0f172a] text-white border-[#0f172a] scale-105 shadow-xl' : 'bg-white hover:border-blue-200'}`}>
                    <div className={`w-16 h-16 rounded-3xl flex items-center justify-center transition-colors ${vehicleStatusFilter === 'soat' ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-600'}`}><Shield size={32} /></div>
                    <div className="text-center"><h4 className="text-4xl font-black tracking-tighter leading-none">{vehicleStats.soat}</h4><p className={`text-[9px] font-black uppercase tracking-[0.2em] mt-2 ${vehicleStatusFilter === 'soat' ? 'text-blue-400' : 'text-slate-400'}`}>SOAT</p></div>
                 </button>
                 <button onClick={() => setVehicleStatusFilter('rtm')} className={`flex flex-col items-center justify-center gap-4 p-8 rounded-[3.5rem] border-2 transition-all duration-300 shadow-sm ${vehicleStatusFilter === 'rtm' ? 'bg-[#0f172a] text-white border-[#0f172a] scale-105 shadow-xl' : 'bg-white hover:border-purple-200'}`}>
                    <div className={`w-16 h-16 rounded-3xl flex items-center justify-center transition-colors ${vehicleStatusFilter === 'rtm' ? 'bg-purple-600 text-white' : 'bg-purple-50 text-purple-600'}`}><Zap size={32} /></div>
                    <div className="text-center"><h4 className="text-4xl font-black tracking-tighter leading-none">{vehicleStats.rtm}</h4><p className={`text-[9px] font-black uppercase tracking-[0.2em] mt-2 ${vehicleStatusFilter === 'rtm' ? 'text-purple-400' : 'text-slate-400'}`}>RTM</p></div>
                 </button>
                 <button onClick={() => setVehicleStatusFilter('plc')} className={`flex flex-col items-center justify-center gap-4 p-8 rounded-[3.5rem] border-2 transition-all duration-300 shadow-sm ${vehicleStatusFilter === 'plc' ? 'bg-[#0f172a] text-white border-[#0f172a] scale-105 shadow-xl' : 'bg-white hover:border-amber-200'}`}>
                    <div className={`w-16 h-16 rounded-3xl flex items-center justify-center transition-colors ${vehicleStatusFilter === 'plc' ? 'bg-amber-600 text-white' : 'bg-amber-50 text-amber-600'}`}><FileText size={32} /></div>
                    <div className="text-center"><h4 className="text-4xl font-black tracking-tighter leading-none">{vehicleStats.plc}</h4><p className={`text-[9px] font-black uppercase tracking-[0.2em] mt-2 ${vehicleStatusFilter === 'plc' ? 'text-amber-400' : 'text-slate-400'}`}>PLC</p></div>
                 </button>
                 <button onClick={() => setVehicleStatusFilter('ext')} className={`flex flex-col items-center justify-center gap-4 p-8 rounded-[3.5rem] border-2 transition-all duration-300 shadow-sm ${vehicleStatusFilter === 'ext' ? 'bg-[#0f172a] text-white border-[#0f172a] scale-105 shadow-xl' : 'bg-white hover:border-orange-200'}`}>
                    <div className={`w-16 h-16 rounded-3xl flex items-center justify-center transition-colors ${vehicleStatusFilter === 'ext' ? 'bg-orange-600 text-white' : 'bg-orange-50 text-orange-600'}`}><Flame size={32} /></div>
                    <div className="text-center"><h4 className="text-4xl font-black tracking-tighter leading-none">{vehicleStats.ext}</h4><p className={`text-[9px] font-black uppercase tracking-[0.2em] mt-2 ${vehicleStatusFilter === 'ext' ? 'text-orange-400' : 'text-slate-400'}`}>EXT</p></div>
                 </button>
              </div>
              ...
              {filteredVehicles.length > 0 ? filteredVehicles.map((vehicle) => (
                <div key={vehicle.id} ref={el => { vehicleRefs.current[vehicle.plate] = el; }} className="bg-white rounded-[3.5rem] border-2 border-slate-100 shadow-xl overflow-hidden flex flex-col xl:flex-row transition-all hover:border-indigo-200">
                  <div className="xl:w-[480px] bg-slate-50/50 p-12 flex flex-col justify-center items-center border-r border-slate-100 shrink-0 relative overflow-hidden">
                    ...
                    <div className="relative z-10 w-full flex flex-col items-center">
                      <div className="w-full px-8 py-12 bg-[#0f172a] text-white rounded-[3rem] font-mono font-black text-6xl tracking-tighter shadow-2xl text-center mb-10 border-[12px] border-white ring-1 ring-slate-100">{vehicle.plate}</div>
                      <div className="space-y-6 w-full max-w-[320px]">
                         <div className="flex items-center gap-5 bg-white p-5 rounded-[1.8rem] shadow-sm border border-slate-100"><div className="p-3 bg-indigo-50 text-indigo-600 rounded-2xl"><MapPin size={22}/></div><div className="flex flex-col"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">CD Asignado</span><span className="text-sm font-black text-slate-800 uppercase">{vehicle.cd || 'ARENOSA'}</span></div></div>
                         <div className="flex items-center gap-5 bg-white p-5 rounded-[1.8rem] shadow-sm border border-slate-100"><div className="p-3 bg-indigo-50 text-indigo-600 rounded-2xl"><Briefcase size={22}/></div><div className="flex flex-col"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Contratista</span><span className="text-sm font-black text-slate-800 uppercase truncate max-w-[200px]">{vehicle.contractor || 'GENERAL'}</span></div></div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="text-center py-40 bg-white rounded-[3.5rem] border-2 border-dashed border-slate-100 shadow-inner opacity-40">
                  ...
                  <p className="text-sm font-black uppercase tracking-[0.4em]">Sin vehículos para este filtro</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeView === 'conductores' && (
            <div className="space-y-12 animate-in fade-in duration-500 max-w-[1400px] mx-auto">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                 <button onClick={() => setDriverStatusFilter('all')} className={`flex flex-col items-center justify-center gap-4 p-8 rounded-[3.5rem] border-2 transition-all duration-300 shadow-sm ${driverStatusFilter === 'all' ? 'bg-[#0f172a] text-white border-[#0f172a] scale-105 shadow-xl' : 'bg-white hover:border-indigo-200'}`}>
                    <div className={`w-16 h-16 rounded-3xl flex items-center justify-center transition-colors ${driverStatusFilter === 'all' ? 'bg-indigo-600 text-white' : 'bg-indigo-50 text-indigo-600'}`}><Users size={32} /></div>
                    <div className="text-center"><h4 className="text-4xl font-black tracking-tighter leading-none">{driverStats.total}</h4><p className={`text-[9px] font-black uppercase tracking-[0.2em] mt-2 ${driverStatusFilter === 'all' ? 'text-indigo-400' : 'text-slate-400'}`}>TOTAL</p></div>
                 </button>
                 <button onClick={() => setDriverStatusFilter('license')} className={`flex flex-col items-center justify-center gap-4 p-8 rounded-[3.5rem] border-2 transition-all duration-300 shadow-sm ${driverStatusFilter === 'license' ? 'bg-[#0f172a] text-white border-[#0f172a] scale-105 shadow-xl' : 'bg-white hover:border-blue-200'}`}>
                    <div className={`w-16 h-16 rounded-3xl flex items-center justify-center transition-colors ${driverStatusFilter === 'license' ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-600'}`}><CreditCard size={32} /></div>
                    <div className="text-center"><h4 className="text-4xl font-black tracking-tighter leading-none">{driverStats.license}</h4><p className={`text-[9px] font-black uppercase tracking-[0.2em] mt-2 ${driverStatusFilter === 'license' ? 'text-blue-400' : 'text-slate-400'}`}>LICENCIA</p></div>
                 </button>
                 <button onClick={() => setDriverStatusFilter('driving')} className={`flex flex-col items-center justify-center gap-4 p-8 rounded-[3.5rem] border-2 transition-all duration-300 shadow-sm ${driverStatusFilter === 'driving' ? 'bg-[#0f172a] text-white border-[#0f172a] scale-105 shadow-xl' : 'bg-white hover:border-indigo-200'}`}>
                    <div className={`w-16 h-16 rounded-3xl flex items-center justify-center transition-colors ${driverStatusFilter === 'driving' ? 'bg-indigo-600 text-white' : 'bg-indigo-50 text-indigo-600'}`}><ShieldCheck size={32} /></div>
                    <div className="text-center"><h4 className="text-4xl font-black tracking-tighter leading-none">{driverStats.driving}</h4><p className={`text-[9px] font-black uppercase tracking-[0.2em] mt-2 ${driverStatusFilter === 'driving' ? 'text-indigo-400' : 'text-slate-400'}`}>CURSO MANEJO</p></div>
                 </button>
                 <button onClick={() => setDriverStatusFilter('medical')} className={`flex flex-col items-center justify-center gap-4 p-8 rounded-[3.5rem] border-2 transition-all duration-300 shadow-sm ${driverStatusFilter === 'medical' ? 'bg-[#0f172a] text-white border-[#0f172a] scale-105 shadow-xl' : 'bg-white hover:border-purple-200'}`}>
                    <div className={`w-16 h-16 rounded-3xl flex items-center justify-center transition-colors ${driverStatusFilter === 'medical' ? 'bg-purple-600 text-white' : 'bg-purple-50 text-purple-600'}`}><Stethoscope size={32} /></div>
                    <div className="text-center"><h4 className="text-4xl font-black tracking-tighter leading-none">{driverStats.medical}</h4><p className={`text-[9px] font-black uppercase tracking-[0.2em] mt-2 ${driverStatusFilter === 'medical' ? 'text-purple-400' : 'text-slate-400'}`}>EXAMENES</p></div>
                 </button>
              </div>
              {filteredDrivers.length > 0 ? filteredDrivers.map(driver => (
                ...
                <div className="text-center py-40 bg-white rounded-[3.5rem] border-2 border-dashed border-slate-100 shadow-inner opacity-40">
                  ...
                  <p className="text-sm font-black uppercase tracking-[0.4em]">Sin conductores registrados para este filtro</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeView === 'kilometrajes' && (
            <div className="max-w-[1600px] mx-auto pb-24 space-y-8 animate-in fade-in duration-500">
               {/* PANEL DE ANALÍTICA KILOMETRAJES COMPACTO */}
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <button 
                   onClick={() => setMileageStatusFilter(prev => prev === 'completed' ? 'all' : 'completed')}
                   className={`flex flex-col items-center justify-center gap-4 p-6 rounded-[2.5rem] border-2 transition-all duration-300 shadow-sm ${mileageStatusFilter === 'completed' ? 'bg-[#0f172a] text-white border-[#0f172a] scale-105 shadow-2xl' : 'bg-white hover:border-emerald-200'}`}
                  >
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors ${mileageStatusFilter === 'completed' ? 'bg-emerald-600 text-white' : 'bg-emerald-50 text-emerald-600'}`}><ListChecks size={28} /></div>
                    <div className="text-center"><h4 className="text-5xl font-black tracking-tighter leading-none">{mileageCompliance.done}</h4><p className={`text-[10px] font-black uppercase tracking-[0.2em] mt-2 ${mileageStatusFilter === 'completed' ? 'text-emerald-400' : 'text-slate-400'}`}>REALIZADOS</p></div>
                  </button>

                  <button 
                   onClick={() => setMileageStatusFilter(prev => prev === 'pending' ? 'all' : 'pending')}
                   className={`flex flex-col items-center justify-center gap-4 p-6 rounded-[2.5rem] border-2 transition-all duration-300 shadow-sm ${mileageStatusFilter === 'pending' ? 'bg-[#0f172a] text-white border-[#0f172a] scale-105 shadow-2xl' : 'bg-white hover:border-rose-200'}`}
                  >
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors ${mileageStatusFilter === 'pending' ? 'bg-rose-600 text-white' : 'bg-rose-50 text-rose-600'}`}><Clock size={28} /></div>
                    <div className="text-center"><h4 className="text-5xl font-black tracking-tighter leading-none">{Math.max(0, mileageCompliance.total - mileageCompliance.done)}</h4><p className={`text-[10px] font-black uppercase tracking-[0.2em] mt-2 ${mileageStatusFilter === 'pending' ? 'text-rose-400' : 'text-slate-400'}`}>PENDIENTES</p></div>
                  </button>

                  <div className="bg-[#0f172a] p-6 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden flex flex-col justify-center border-2 border-indigo-600/20">
                    <div className="absolute top-0 right-0 p-4 opacity-5"><Target size={100} /></div>
                    <div className="relative z-10">
                       <div className="flex items-center justify-between mb-4">
                          <div><h2 className="text-xl font-black uppercase tracking-tighter leading-none mb-1">Efectividad</h2><p className="text-indigo-400 font-bold uppercase tracking-widest text-[8px]">REPORTE SEMANAL</p></div>
                          <div className="bg-indigo-600 px-4 py-2 rounded-2xl shadow-xl"><span className="text-2xl font-black">{mileageCompliance.percentage}%</span></div>
                       </div>
                       <div className="w-full bg-white/10 h-4 rounded-full overflow-hidden border border-white/5 mb-4 p-0.5"><div className="h-full bg-gradient-to-r from-indigo-500 to-emerald-500 rounded-full transition-all duration-1000" style={{width: `${mileageCompliance.percentage}%`}}></div></div>
                       <div className="flex items-center justify-between"><div className="flex items-center gap-2"><div className="p-1.5 bg-white/5 rounded-lg"><Truck size={12} className="text-indigo-400" /></div><span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{mileageCompliance.done} / {mileageCompliance.total} Camiones</span></div><div className="flex items-center gap-1 text-emerald-400 text-[8px] font-black uppercase tracking-widest"><CheckCircle size={10} /> Activo</div></div>
                    </div>
                  </div>
               </div>

               <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-lg">
                  ...
                  <div className="hidden md:flex items-center gap-3 px-4 py-2 bg-slate-50 rounded-xl border border-slate-100">
                     ...
                     <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">MONITOREO DE KILOMETRAJE S{selectedWeek}</span>
                  </div>
               </div>
               ...
               <div className="mt-8 bg-white p-8 rounded-[3rem] border border-slate-100 shadow-xl overflow-hidden">
                  <div className="flex items-center justify-between mb-8">
                     <div className="flex items-center gap-3"><div className="p-2.5 bg-indigo-50 text-indigo-600 rounded-xl"><History size={20} /></div><h2 className="text-2xl font-black uppercase tracking-tighter">Historial</h2></div>
                     <ExportButton data={filteredMileageLogs} filename="KILOMETRAJES" title="Exportar" />
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-separate border-spacing-y-2">
                        <thead><tr className="text-slate-400"><th className="py-2 px-6 text-[10px] font-black uppercase tracking-widest">Fecha</th><th className="py-2 px-6 text-[10px] font-black uppercase tracking-widest">Placa</th><th className="py-2 px-6 text-[10px] font-black uppercase tracking-widest text-right">Kilometraje</th><th className="py-2 px-6 text-[10px] font-black uppercase tracking-widest">CD</th></tr></thead>
                        <tbody>{(filteredMileageLogs || []).map((log, idx) => (<tr key={idx} className="bg-slate-50/50 hover:bg-white hover:shadow-lg transition-all group rounded-2xl"><td className="py-4 px-6 rounded-l-2xl"><span className="text-[11px] font-bold text-slate-500">{formatDate(log.date)}</span></td><td className="py-4 px-6"><span className="px-4 py-1.5 bg-[#0f172a] text-white rounded-xl font-mono font-black text-sm tracking-tighter">{log.plate}</span></td><td className="py-4 px-6 text-right"><span className="text-xl font-black text-indigo-600 tracking-tighter">{log.mileage.toLocaleString()} KM</span></td><td className="py-4 px-6 rounded-r-2xl"><span className="text-[10px] font-black text-slate-800 uppercase tracking-tight">{log.cd}</span></td></tr>))}</tbody>
                      </table>
                  </div>
               </div>
            </div>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 bg-[#0f172a] p-10 rounded-[3rem] text-white flex items-center gap-10 shadow-2xl">
                    <div className="relative w-32 h-32 flex items-center justify-center">
                        <svg className="w-full h-full transform -rotate-90"><circle cx="64" cy="64" r="58" stroke="currentColor" strokeWidth="12" fill="transparent" className="text-white/10" /><circle cx="64" cy="64" r="58" stroke="currentColor" strokeWidth="12" fill="transparent" strokeDasharray={364} strokeDashoffset={364 - (364 * reportStats.percentage) / 100} className="text-indigo-400" strokeLinecap="round" /></svg>
                  ...
                  <div className="bg-white p-10 rounded-[3rem] border-2 border-slate-100 flex flex-col justify-center items-center shadow-xl">
                    <button onClick={() => setShowReportForm(true)} className="w-full py-6 bg-indigo-600 text-white rounded-[2rem] font-black text-xs uppercase tracking-widest shadow-xl hover:bg-indigo-700 transition-all flex items-center justify-center gap-4"><Plus size={24} /> NUEVO REPORTE</button>
                  </div>
               </div>
               <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10 pb-16">
                  {filteredReports.length > 0 ? filteredReports.map(report => (<ReportCard key={report.id} report={report} onViewDoc={(url, title) => setViewerDoc({url, title})} onManageClosure={(r) => setClosureReport(r)} />)) : (<div className="col-span-full text-center py-40 opacity-40"><ClipboardList size={64} className="mx-auto mb-6" /><p className="text-sm font-black uppercase tracking-[0.4em]">Sin reportes registrados</p></div>)}
               </div>
            </div>
          )}
            ...
            <div className="space-y-12 animate-in fade-in duration-500 max-w-[1400px] mx-auto pb-24">
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 bg-emerald-600 p-10 rounded-[3rem] text-white flex items-center gap-10 shadow-2xl">
                     <div className="p-8 bg-white/10 rounded-[2.5rem] border border-white/20"><span className="text-6xl font-black">{fiveSCompliance.percentage}%</span></div>
                     <div><h2 className="text-2xl font-black uppercase tracking-tighter">Cobertura de Auditoría</h2><p className="text-emerald-100 font-bold uppercase tracking-widest text-[10px] mb-4">Alcance de inspecciones 5S</p><div className="flex items-center gap-3"><ShieldCheck size={20} className="text-emerald-200" /><span className="text-[11px] font-black uppercase tracking-widest">{fiveSCompliance.audited} de {fiveSCompliance.total} camiones auditados</span></div></div>
                  </div>
                  <div className="bg-white p-8 rounded-[3rem] border-2 border-slate-100 flex flex-col justify-center items-center shadow-xl">
                    <button onClick={() => setShowFiveSForm(true)} className="w-full py-6 bg-[#0f172a] text-white rounded-[2rem] font-black text-xs uppercase tracking-widest shadow-2xl hover:bg-emerald-600 transition-all flex items-center justify-center gap-3"><Plus size={24} /> NUEVA AUDITORÍA</button>
                  </div>
               </div>
