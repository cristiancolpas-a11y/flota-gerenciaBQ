...
const ReportForm: React.FC<ReportFormProps> = ({ onClose, onSubmit, vehicles }) => {
  ...
  const handleAddPhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !formData.plate) {
      if (!formData.plate) alert("Seleccione la placa antes de tomar la foto para estampar el registro.");
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = { ...formData, initialEvidence: mergedInitialEvidence, status: 'ABIERTO' };
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8 bg-white">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input type="date" className="w-full bg-slate-50 border-2 border-slate-200 rounded-2xl px-5 py-4 text-sm font-black text-slate-800" value={formData.date} onChange={e => setFormData({ ...formData, date: e.target.value })} />
            </div>
            <div className="space-y-2">
              <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                <ImageIcon size={14} /> Placa Vehicular
              </label>
              <select required className="w-full bg-slate-50 border-2 border-slate-200 rounded-2xl px-5 py-4 text-sm font-black text-slate-800 outline-none focus:border-indigo-500 transition-all appearance-none" value={formData.plate} onChange={e => setFormData({ ...formData, plate: e.target.value })}>
          ...
          <div className="space-y-2">
            <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Taller y Descripción</label>
            ...
            <textarea required rows={2} placeholder="Describa el hallazgo..." className="w-full border-2 border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:border-indigo-500 resize-none shadow-inner" value={formData.novelty} onChange={e => setFormData({ ...formData, novelty: e.target.value })} />
          </div>
          ...
          <div className="space-y-2">
            <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Mapa de Ingreso</label>
            <button type="button" onClick={() => mapInputRef.current?.click()} className={`w-full py-5 rounded-2xl border-2 border-dashed flex items-center justify-center gap-3 transition-all ${formData.entryMap ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-slate-50 border-slate-300 text-slate-500'}`}>
              <MapPin size={24} /> 
              <span className="text-[10px] font-black uppercase tracking-widest">{formData.entryMap ? 'MAPA CAPTURADO' : 'MAPA DE INGRESO'}</span>
            </button>
          ...
          <button type="submit" disabled={isSubmitting || isProcessingPhoto} className="w-full py-6 bg-[#0f172a] text-white font-black rounded-[2rem] text-sm uppercase shadow-2xl hover:bg-indigo-600 disabled:opacity-50 transition-all flex items-center justify-center gap-4 group">
            ...
            {isSubmitting ? 'VINCULANDO...' : 'GUARDAR Y ABRIR NOVEDAD'}
          </button>
        </form>
      </div>
    </div>
