...
import { Settings2, Calendar, Eye, Clock, ShieldCheck, Scale } from 'lucide-react';
    ...
    <div className={`p-8 rounded-[2.5rem] border-2 transition-all hover:shadow-2xl hover:-translate-y-1 bg-white group ${s.border}`}>
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="flex flex-col items-end">
          ...
          {calibration.daysPending !== undefined && (
      ...
      <div className="space-y-6">
        <div>
          <h3 className="text-sm font-black text-slate-900 uppercase tracking-tighter mb-1">{calibration.equipment}</h3>
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 bg-[#0f172a] text-white rounded-lg font-mono font-black text-[10px] tracking-tight">{calibration.plate}</span>
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest truncate max-w-[150px]">{calibration.cd}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100/50">
             ...
             <span className="text-[10px] font-black text-slate-700">{formatDate(calibration.calibrationDate)}</span>
          </div>
          <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100/50">
             <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-1">Vencimiento</span>
             <span className="text-[10px] font-black text-slate-700">{formatDate(calibration.expiryDate)}</span>
          </div>
        </div>
        ...
        {calibration.certificateUrl ? (
          <button 
            ...
            className="w-full flex items-center justify-center gap-3 py-4 bg-[#0f172a] text-white rounded-2xl text-[10px] font-black transition-all uppercase tracking-[0.2em] shadow-lg hover:bg-indigo-600"
          >
