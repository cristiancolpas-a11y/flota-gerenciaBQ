...
const WorkshopEntryForm: React.FC<WorkshopEntryFormProps> = ({ report, onClose, onSubmit }) => {
  ...
  const handleMapCapture = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setFormData(prev => ({ ...prev, entryMap: reader.result as string }));
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.entryMap) {
      alert('Debe adjuntar el mapa de ingreso.');
      return;
    }
  ...
  return (
    <div className="fixed inset-0 bg-[#0f172a]/90 backdrop-blur-sm z-[100] flex items-center justify-center p-4 overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8 bg-white">
          ...
          <div className="space-y-2">
            ...
            <button type="button" onClick={() => mapEntryInputRef.current?.click()} className={`w-full py-5 rounded-2xl border-4 border-dashed flex flex-col items-center justify-center gap-1 transition-all shadow-sm ${formData.entryMap ? 'bg-indigo-50 border-indigo-500 text-indigo-600' : 'bg-slate-50 border-slate-200 text-slate-400'}`}>
              ...
              <span className="text-[10px] font-black uppercase tracking-widest">{formData.entryMap ? 'MAPA CAPTURADO ✓' : 'SELECCIONAR EL MAPA'}</span>
            </button>
            <input type="file" accept="image/*,image/heic,image/heif,image/jpeg,image/png,image/webp" ref={mapEntryInputRef} className="hidden" onChange={handleMapCapture} />
          </div>
